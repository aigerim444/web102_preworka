...
function addGamesToPage(games) {
    ...
    for (let i = 0; i < games.length; i++){
        ...
        gameElem.classList.add("game-card");
...
const contributionsCard = document.getElementById("num-contributions"); //grabbing the HTML element to update

// use reduce() to count the number of total contributions by summing the backers
const totalContribution = GAMES_JSON.reduce((total, game) => total + game.backers, 0);

// set the inner HTML using a template literal and toLocaleString to get a number with commas
let strTotalContribution = totalContribution.toLocaleString('en-US');
contributionsCard.innerHTML = `${strTotalContribution}`;

// grab the amount raised card, then use reduce() to find the total amount raised
const raisedCard = document.getElementById("total-raised");
const totalRaised = GAMES_JSON.reduce((total,game) => total + game.pledged, 0);
// set inner HTML using template literal
let strtotalRaised = totalRaised.toLocaleString('en-US');
raisedCard.innerHTML = `${strtotalRaised}`;
...
const unfundedStr = `A total of $${totalMoney.toLocaleString('en-US')} have been raised for ${totalGames} games. 
    ${numUnfunded === 1 ? "Currently, 1 game remains unfunded." : 
    `Currently, ${numUnfunded} games remain unfunded.`} `;
